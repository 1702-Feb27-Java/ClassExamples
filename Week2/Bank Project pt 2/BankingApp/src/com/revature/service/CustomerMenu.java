...
public class CustomerMenu {
	...
	public static void checkBalance(String username) {
		...
		// we get all accounts of one user
		...
		// but if a user doesn't have accounts, then return them into the customer menu
		if (allUserAccounts.isEmpty()){
			System.out.println("You have not opened any accounts! Returning you to the customer menu.");
			MenuClass.showCustomerMenu();
			CustomerMenu.functionality(username);
		} else {
	...
	public static void withdraw(String username) {
		...
		try {
			...
			// get the current account as an accountclass
			
			AccountClass currentAccount = new AccountClass();
			currentAccount = daoAccount.getAccountByID(accountID);
			
			// if status = 1, then account not approved
	...
	// can be called as many times as the user likes
